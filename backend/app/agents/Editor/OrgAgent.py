...
class OrgAgent:
    ...
    def create_layout_analyzer_agent(self):
        ...
        return Agent(
            ...
            backstory="""당신은 15년간 세계 최고 수준의 매거진 디자인 스튜디오에서 활동해온 레이아웃 아키텍트입니다. Pentagram, Sagmeister & Walsh, 그리고 Condé Nast의 아트 디렉터로 활동하며 수백 개의 수상작을 디자인했습니다.
            ...
            **PDF 벡터 데이터 활용 전문성:**
            당신은 텍스트 배치 결정 시 다음 벡터 데이터들을 체계적으로 분석합니다:

            1. **레이아웃 패턴 벡터 분석**:
            - 5000개 이상의 매거진 페이지에서 추출한 텍스트 배치 패턴
            - 텍스트 블록 크기, 위치, 간격의 황금비율 데이터
            - 독자 시선 흐름과 텍스트 배치의 상관관계 벡터
            - 매거진 카테고리별 최적 레이아웃 패턴 클러스터링

            2. **타이포그래피 벡터 데이터**:
            - 폰트 크기, 행간, 자간의 최적 조합 벡터
            - 텍스트 길이와 가독성의 상관관계 데이터
            - 제목-부제목-본문의 계층적 구조 패턴
            - 언어별, 문화권별 텍스트 선호도 벡터

            3. **콘텐츠 특성 벡터 매칭**:
            - 텍스트의 감정적 톤과 레이아웃 스타일의 조화도
            - 콘텐츠 길이와 최적 페이지 구성의 상관관계
            - 독자 연령대별 선호 레이아웃 패턴
            - 계절성 및 여행 테마별 레이아웃 트렌드

            **작업 방법론:**
            "나는 단순히 아름다운 레이아웃을 만드는 것이 아니라, 과학적 데이터에 기반하여 독자의 인지 부하를 최소화하고 정보 전달 효율을 극대화하는 레이아웃을 설계합니다. 수천 개의 성공적인 매거진 페이지에서 추출한 벡터 패턴을 분석하여, 각 텍스트 콘텐츠가 가진 고유한 특성에 가장 적합한 레이아웃을 찾아냅니다."

            **학습 데이터 활용 전략:**
            - 이전 레이아웃 결정의 독자 반응 데이터를 분석하여 벡터 가중치 조정
            - 새로운 매거진 트렌드를 벡터 데이터베이스에 지속적으로 업데이트
            - 다른 에이전트들의 피드백을 통한 텍스트-이미지 조화도 개선
            - A/B 테스트 결과를 통한 레이아웃 효과성 검증 및 모델 개선""",
            llm=self.llm,
            verbose=True
        )
    ...
    async def process_content(self, magazine_content, available_templates: List[str]) -> Dict:
        ...
        print(
            f"OrgAgent: 처리할 콘텐츠 - {len(all_content)}자, {len(content_sections)}개 섹션")
        ...
        for i, section_content in enumerate(content_sections):
            ...
            try:
                ...
                # 결과 파싱
                analysis_result = str(layout_analysis_task.output) if hasattr(
                    layout_analysis_task, 'output') else ""
                edited_content = str(result.raw) if hasattr(
                ...
                print(
                    f"✅ 섹션 {i+1} 편집 완료: {len(section_content)}자 → {len(clean_content)}자")
        ...
        # 템플릿 매핑
        text_mapping = self._map_to_templates(
            refined_sections, available_templates)

        total_refined_length = sum(
            section["refined_length"] for section in refined_sections)
        print(
            f"✅ OrgAgent 완료: {len(refined_sections)}개 섹션, 총 {total_refined_length}자")
    ...
    def _extract_clean_title_subtitle(self, analysis_result: str, index: int) -> tuple:
        ...
        ) if title_match else f"도쿄 여행 이야기 {index + 1}"
        subtitle = subtitle_match.group(
            1).strip() if subtitle_match else "특별한 순간들"
    ...
    def _remove_meta_descriptions(self, content: str) -> str:
        ...
        for pattern in patterns_to_remove:
            clean_content = re.sub(
                pattern, '', clean_content, flags=re.IGNORECASE | re.DOTALL)
    ...
    def _map_to_templates(self, refined_sections: List[Dict], available_templates: List[str]) -> List[Dict]:
        ...
        for i, section in enumerate(refined_sections):
            template_name = available_templates[i] if i < len(
                available_templates) else f"Section{i+1:02d}.jsx"
