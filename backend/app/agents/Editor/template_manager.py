...
class MultiAgentTemplateManager:
    ...
    async def initialize_vector_system(self, template_folder: str = "templates"):
        ...
        await self.vector_manager.initialize_search_index()

        # PDF 템플릿 처리 및 벡터화
        await self.vector_manager.process_pdf_templates(template_folder)
    ...
    async def should_initialize_vector_system(self) -> bool:
        ...
        try:
            ...
            await index_client.get_index(self.vector_manager.search_index_name)
    ...
    def get_available_templates(self):
        ...
        template_files = [f for f in os.listdir(
            templates_dir) if f.endswith('.jsx')]
    ...
    async def create_magazine_data(self, magazine_content, image_analysis_results: List[Dict]) -> Dict:
        ...
        if await self.should_initialize_vector_system():
            print("\n=== PDF 벡터 시스템 초기화 (필요한 경우만) ===")
            await self.vector_manager.process_pdf_templates("templates")
        else:
            print("\n=== 기존 벡터 데이터 사용 ===")

        # 기본 데이터 준비
        available_templates = self.get_available_templates()
        template_requirements = self.analyze_template_requirements(
            available_templates)

        image_urls = [result.get(
            'image_url', '') for result in image_analysis_results if result.get('image_url')]
        image_locations = [result.get(
            'location', '') for result in image_analysis_results if result.get('location')]
        ...
        final_template_data["pdf_sources"] = self._extract_pdf_sources(
    ...
    async def generate_react_app(self, template_data: Dict, file_manager, project_name: str):
        """React 앱 생성"""
        project_folder = await file_manager.create_project_folder(project_name)
        src_folder, components_folder = await file_manager.create_react_app(project_folder)

        # template_data.json 저장
        template_data_path = os.path.join(project_folder, "template_data.json")
        await file_manager.save_json(template_data, template_data_path)

        # 벡터 데이터 메타정보 저장
        if template_data.get("vector_enhanced"):
            vector_info_path = os.path.join(
                project_folder, "vector_sources.json")
            vector_info = {
            ...
            await file_manager.save_json(vector_info, vector_info_path)
