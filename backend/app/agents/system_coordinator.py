...
# Cosmos DB 관련 import
from db.cosmos_connection import image_container, magazine_container
...
class SystemCoordinator:
    ...
    async def _execute_content_creation_stage(self, image_results: List[Dict]) -> str:
        ...
        try:
            ...
            # ✅ Cosmos DB에 저장
            # magazine_content가 문자열이면 dict로 변환
            ...
            # mag_id 필드가 없으면 생성 (예: "unknown_mag" 또는 고유값)
            ...
            if isinstance(magazine_content, str):
                try:
                    ...
                    if isinstance(parsed_json, dict) and "sections" in parsed_json:
                        # 이미 섹션 구조가 있는 경우
                        self.logger.info(
                            f"기존 섹션 구조 발견: {len(parsed_json['sections'])}개")
                        return parsed_json
