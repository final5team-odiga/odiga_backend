...
{chr(10).join(image_tags)}
    ...
    def _ensure_content_inclusion(self, jsx_code: str, content: Dict) -> str:
        ...
        if title and title not in jsx_code:
            ...
            jsx_code = jsx_code.replace('<Title></Title>', f'<Title>{title}</Title>')
        ...
        if subtitle and subtitle not in jsx_code:
            ...
            jsx_code = jsx_code.replace('<Subtitle></Subtitle>', f'<Subtitle>{subtitle}</Subtitle>')
        
        if body and body not in jsx_code:
            jsx_code = jsx_code.replace('{body}', body)
            jsx_code = jsx_code.replace('<Content></Content>', f'<Content>{body}</Content>')
    ...
    def _preserve_structure_adapt_content(self, original_jsx: str, template_info: Dict, content: Dict, component_name: str) -> str:
        ...
        if props:
            ...
            for prop in props:
                prop = prop.strip()
                if prop == 'title':
                    adapted_jsx = adapted_jsx.replace(f'{{{prop}}}', content.get('title', '도쿄 여행 이야기'))
                elif prop == 'subtitle':
                    adapted_jsx = adapted_jsx.replace(f'{{{prop}}}', content.get('subtitle', '특별한 순간들'))
                elif prop == 'body':
                    adapted_jsx = adapted_jsx.replace(f'{{{prop}}}', content.get('body', '여행의 아름다운 기억들'))
                elif prop == 'tagline':
                    adapted_jsx = adapted_jsx.replace(f'{{{prop}}}', content.get('tagline', 'TRAVEL & CULTURE'))
    ...
    def _apply_vector_style_enhancements(self, jsx_code: str, template_info: Dict) -> str:
        ...
        color_enhancements = self._extract_colors_from_vector_data(similar_layouts)
