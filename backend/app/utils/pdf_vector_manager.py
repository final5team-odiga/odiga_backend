...
class PDFVectorManager:
    ...
    def __init__(self):
        ...
        self.form_recognizer_endpoint = os.getenv(
    ...
    def initialize_search_index(self):
        ...
        try:
            # 1. 기존 인덱스 존재 여부 확인
            try:
                existing_index = self.search_index_client.get_index(
                ...
                if self._check_index_has_data():
                    print(
                        f"✅ 인덱스에 데이터 존재 ({self._get_document_count()}개 문서) - 벡터 처리 생략")
            ...
            fields = [
                ...
                SimpleField(name="content_type",
                            type=SearchFieldDataType.String, filterable=True),
                SimpleField(name="text_content",
                            type=SearchFieldDataType.String, searchable=True),
                SimpleField(name="layout_info",
                            type=SearchFieldDataType.String),
                SimpleField(name="image_info",
                            type=SearchFieldDataType.String),
                SearchField(
                    name="content_vector",
    ...
    def process_pdf_templates(self, template_folder: str = "templates"):
        ...
        # 1단계: 큰 PDF 파일들 분할
        print("\n🔪 PDF 분할 단계")
        available_pdf_files = self.pdf_splitter.split_large_pdfs(
            template_folder)
        ...
        new_pdfs = [
            pdf for pdf in available_pdf_files if pdf not in processed_pdfs]
    ...
    async def search_similar_layouts(self, query_text: str, content_type: str = None, top_k: int = 5) -> List[Dict]:
        """유사한 레이아웃 검색"""
    ...
    def check_pdf_compatibility(self, template_folder: str):
        ...
        pdf_files = [f for f in os.listdir(
            template_folder) if f.endswith('.pdf')]
